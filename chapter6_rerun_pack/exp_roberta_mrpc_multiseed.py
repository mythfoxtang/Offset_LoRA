...
def build_model(model_id, mode, rank, alpha):
    ...
    for name, module in model.named_modules():
        if any(token in name for token in ["query", "value"]) and isinstance(module, nn.Linear):
            parts = name.rsplit(".", 1)
...
def main():
    ...
    parser.add_argument("--max-steps", type=int, default=0, help="0 means full pass")
    parser.add_argument("--model-id", default="AI-ModelScope/roberta-base")
    parser.add_argument("--output-root", default="results/chapter6_rerun")
    ...
    nan_step = None
    model.train()
    pbar = tqdm(range(0, len(train_ds), args.batch_size), desc=f"mrpc {args.mode} lr={args.lr} seed={args.seed}")
    ...
    out_name = f"roberta_mrpc_{args.mode}_lr{safe_tag(args.lr)}_seed{args.seed}.json"
